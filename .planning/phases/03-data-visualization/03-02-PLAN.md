...
must_haves:
  truths:
    - "Axes::new(x, y, width, height) creates a plot container with auto-ranging enabled by default"
    - "Axes::add_curve() attaches a DataCurve to the axes"
    - "Axes::x_range(min, max) and y_range(min, max) override auto-range"
    - "Axes::to_primitives() returns Vec<Primitive> including axis lines, tick marks, tick labels, grid lines, and curve paths"
    - "Heckbert nice-numbers generates 5–10 human-readable tick values with no floating-point label noise"
    - "Auto-range uses 7% padding beyond data extent; degenerate data (single value) uses ±0.5 span"
    - "SVG Y-axis inversion is correct: data Y increases upward, pixel Y increases downward"
    - "Axis titles (optional) render centered below X axis and rotated beside Y axis"
  artifacts:
    - path: "src/dataviz/axes.rs"
      provides: "Axes struct, AxisRange enum, Heckbert tick generation, coordinate mapping, to_primitives()"
      exports: ["Axes", "AxisRange"]
      min_lines: 200
    - path: "src/dataviz/mod.rs"
      provides: "pub mod axes; pub use axes::{Axes, AxisRange} added"
      min_lines: 8
  key_links:
    - from: "src/dataviz/axes.rs"
      to: "src/dataviz/data_curve.rs"
      via: "Axes.curves: Vec<DataCurve>; to_primitives() calls curve.to_bezier_path(visual_pts)"
      pattern: "DataCurve"
    - from: "src/dataviz/axes.rs"
      to: "src/primitives/mod.rs"
      via: "to_primitives() returns Vec<crate::primitives::Primitive>"
      pattern: "crate::primitives::Primitive"
    - from: "src/dataviz/axes.rs"
      to: "src/primitives/line.rs"
      via: "Axis lines and tick marks created as Line primitives"
      pattern: "crate::primitives::Line"
    - from: "src/dataviz/axes.rs"
      to: "src/primitives/text.rs"
      via: "Tick labels and axis titles created as Text primitives"
      pattern: "crate::primitives::Text"
---

<objective>
Implement the Axes struct with Heckbert tick generation, coordinate mapping, auto-ranging, and to_primitives() decomposition.

Purpose: DATA-01 (cartesian axes) and DATA-03 (auto-range) are both implemented here. Axes is not a Primitive variant — it decomposes via to_primitives() into Lines (axis lines, tick marks, grid lines) and Texts (tick labels, optional axis titles), then the attached DataCurve paths. This keeps svg_gen.rs and the Primitive enum unchanged.

Output: src/dataviz/axes.rs, and updated src/dataviz/mod.rs.
</objective>
...
<task type="auto">
  <name>Task 1: Implement Axes struct with auto-range and Heckbert tick generation</name>
  <files>src/dataviz/axes.rs</files>
  <action>
Create src/dataviz/axes.rs with the full Axes implementation. Key locked decisions:
- Auto-tick only (5–10 ticks, no user override of count)
- 7% padding on auto-range (within locked 5–10% range)
- Degenerate data: use [value - 0.5, value + 0.5]
- Grid lines at tick positions (subtle, dashed)
- Tick labels as plain decimals (no scientific notation)
- Optional x_title / y_title
- X and Y axis lines only (not all 4 sides — minimalist scientific style per research open question resolution)
- Axes are static only for Phase 3
...
impl Axes {
    ...
    pub fn to_primitives(&self) -> Vec<Primitive> {
        ...
        if let Ok(x_line) = Line::new(self.x, x_axis_y, self.x + self.width, x_axis_y)
            .stroke(Color::WHITE, AXIS_STROKE_WIDTH)
        {
            prims.push(x_line.into());
        }

        // Y axis line (vertical, at left)
        if let Ok(y_line) = Line::new(y_axis_x, self.y, y_axis_x, self.y + self.height)
            .stroke(Color::WHITE, AXIS_STROKE_WIDTH)
        {
        ...
        for &tick_val in &x_ticks {
            ...
            if let Ok(tick) = Line::new(px, x_axis_y, px, x_axis_y + TICK_LENGTH)
                .stroke(Color::WHITE, TICK_STROKE_WIDTH)
            {
            ...
            let label_text = Text::new(label, px, x_axis_y + TICK_LENGTH + TICK_LABEL_OFFSET, TICK_LABEL_SIZE);
            prims.push(label_text.into());

            // Grid line (vertical, from x axis up to top of plot)
            if let Ok(grid) = Line::new(px, self.y, px, self.y + self.height)
                .stroke(Color::new(180, 180, 180), GRID_STROKE_WIDTH)
            {
                // Grid lines need opacity; apply via Bezier to avoid adding opacity to Line builder
                // Use a dashed Bezier path for the grid line
                ...
                drop(grid);  // unused line — used Bezier instead for opacity control
            }
        }
        ...
        for &tick_val in &y_ticks {
            ...
            if let Ok(tick) = Line::new(y_axis_x - TICK_LENGTH, py, y_axis_x, py)
                .stroke(Color::WHITE, TICK_STROKE_WIDTH)
            {
                prims.push(tick.into());
            }
            ...
            let label_text = Text::new(label, y_axis_x - TICK_LENGTH - TICK_LABEL_OFFSET * 0.6, py + TICK_LABEL_SIZE * 0.35, TICK_LABEL_SIZE);
            prims.push(label_text.into());
        ...
        if let Some(ref title) = self.x_title {
            ...
            prims.push(Text::new(title.clone(), title_x, title_y, AXIS_TITLE_SIZE).into());
        }

        if let Some(ref title) = self.y_title {
            // Rotated beside Y axis — render as Text at rotated position
            // SVG rotation is applied in Text::to_svg_element(); for now place at left center.
            // Positioned to the left of y-axis; rotation handled by SVG transform if Text supports it.
...
**Then update src/dataviz/mod.rs** to add axes:
```rust
pub mod axes;
pub mod data_curve;
...
IMPORTANT — SVG Y-axis inversion: map_y() maps data_min to the BOTTOM pixel (axes.y + height) and data_max to the TOP pixel (axes.y). This is the opposite of map_x(). Without this flip, all curves render upside down.

IMPORTANT — Grid line opacity: The existing Line primitive does not have an opacity field. Use Bezier (which has opacity) for grid lines to achieve the subtle GRID_OPACITY=0.15 appearance.

IMPORTANT — Catmull-Rom in pixel space: In to_primitives() Step 7, map data points to visual_pts FIRST, then call curve.to_bezier_path(&visual_pts). Never call to_bezier_path on data-space coordinates.

IMPORTANT — Tick label precision: Use format_tick(val, step) not val.to_string(). The step is computed from the tick array; derive it as ticks[1] - ticks[0].
  </action>
  <verify>
    <automated>cd /Users/reallimoges/repositories/eidos && cargo test --lib dataviz::axes 2>&1 | tail -30</automated>
    <manual>Check that all 11 unit tests pass. Confirm map_y returns axes.y + height for data_min and axes.y for data_max. Confirm format_tick("0.3" test) passes.</manual>
    <sampling_rate>run after this task, before Plan 03</sampling_rate>
  </verify>
  <done>
    - src/dataviz/axes.rs compiles with zero errors, zero warnings
    - src/dataviz/mod.rs exports Axes, AxisRange, DataCurve
    - All 11 unit tests pass: tick generation, tick formatting, coordinate mapping (x and y), auto-range, degenerate data, to_primitives produces primitives, explicit range override
    - map_y() inverts the Y axis (data_min → bottom, data_max → top)
    - Heckbert tick labels use format_tick() — "0.3" not "0.30000000000000004"
    - Grid lines use Bezier with opacity=0.15 (not plain Line, which has no opacity field)
  </done>
</task>
...
<verification>
cargo test --lib dataviz 2>&1 — all DataCurve + Axes tests green (17+ tests total)
cargo build 2>&1 — dataviz module compiles (not yet wired into lib.rs — expected until Plan 03)
</verification>

<success_criteria>
- Axes struct with new(), x_range(), y_range(), x_title(), y_title(), add_curve(), to_primitives()
- AxisRange enum (Auto, Explicit)
- Heckbert nice-numbers generates round tick values with no floating-point label noise
- map_y() correctly inverts SVG Y axis
- Auto-range with 7% padding; degenerate ±0.5 span
- Explicit range override works
- Zero new external dependencies
</success_criteria>

<output>
After completion, create `.planning/phases/03-data-visualization/03-02-SUMMARY.md`
</output>
